exports.execute = async (featureModel) => {
    ...
    return await numberGroupesOr(featureTree);
}

const numberGroupesOr = async (featureTree) => {
    let response = 0;
    ...
    if (featureTree.children) {
        for (let i = 0; i < featureTree.children.length; i++) {
            if (featureTree.children[i].type == 'g' && featureTree.children[i].multiplicity == '1,*')
                response += await numberGroupesOr(featureTree.children[i]) + 1;
            else
                response += await numberGroupesOr(featureTree.children[i]);
        }
    }
